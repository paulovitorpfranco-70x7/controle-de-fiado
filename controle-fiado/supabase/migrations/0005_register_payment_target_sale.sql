...
begin
  ...
  if p_target_sale_id is not null then
    select id, remaining_amount_cents, due_date
    into v_sale
    from public.sales
    where id = p_target_sale_id
      and customer_id = p_customer_id
    ...
    if found and v_remaining_payment > 0 then
      ...
      if v_allocated_amount > 0 then
        insert into public.payment_allocations (payment_id, sale_id, amount_cents)
        values (v_payment_id, v_sale.id, v_allocated_amount);
